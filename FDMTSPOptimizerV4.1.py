#Author: Jesse Evans
#Version 3.0
#This python code will accept a unoptimized Gcode file, and apply a TSP optimizer to help gain better performance in printing times.
...
class SubProblem:
  ...
  def Solve(self, Start, End, Open = False):
    ...
    DistanceMatrix = []
    for line in self.Lines:
      lineIndex = self.Lines.index(line)
      Eval1 = []
      Eval2 = []
      ...
      for Node in DistanceMatrix:
        ...
        if NodeIndex %2 == 0:
          d1 = GetDistance(self.Lines[int(NodeIndex/2)].XValue[0],self.Lines[int(NodeIndex/2)].YValue[0],self.Lines[int(lineIndex/2)].XValue[0],self.Lines[int(lineIndex/2)].YValue[0])
          d2 = GetDistance(self.Lines[int(NodeIndex/2)].XValue[0],self.Lines[int(NodeIndex/2)].YValue[0],self.Lines[int(lineIndex/2)].XValue[1],self.Lines[int(lineIndex/2)].YValue[1])
          Node.append(d1)
          Node.append(d2)
          Eval1.append(d1)
          Eval2.append(d2)
        else:
          d1 = GetDistance(self.Lines[int((NodeIndex - 1)/2)].XValue[1],self.Lines[int((NodeIndex - 1)/2)].YValue[1],self.Lines[int(lineIndex/2)].XValue[0],self.Lines[int(lineIndex/2)].YValue[0])
          d2 = GetDistance(self.Lines[int((NodeIndex - 1)/2)].XValue[1],self.Lines[int((NodeIndex - 1)/2)].YValue[1],self.Lines[int(lineIndex/2)].XValue[1],self.Lines[int(lineIndex/2)].YValue[1])
          Node.append(d1)
          Node.append(d2)
          Eval1.append(d1)
          Eval2.append(d2)
    ...
    NewOrder = []
    for x in range(0,len(Assignment),2):
      if(x %2 != 0 ):
...
class LayerProblem:
  ...
  def Solve(self, FirstLayer = False, EVal = 0):
    ...
    for Problem in self.SubProblems:
      ...
      for i in range(Size):
        if i%100 == 0:
          print("Element {} in problemset {} added".format(i,self.SubProblems.index(Problem)))
        Elem = []
        Elem2 = []
...
if __name__ ==  "__main__":
  filename = "./UM3E_20mmTestCube_repaired.gcode"#sys.argv[1]
